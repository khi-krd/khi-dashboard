...
function isRenderablePreview(url: string | null): boolean {
...
export function CollectionCoverSlot({
  label,
  helper = NS.cover.helper,
  highlightedHint,
  isHoverSlot,
  inactive,
  previewUrl,
  ...
  const preview = isRenderablePreview(previewUrl) ? previewUrl!.trim() : null
  ...
  return (
    ...
    >
      <p className="text-muted-foreground flex flex-wrap items-center gap-1 text-xs font-medium">
        {isHoverSlot ? (
          <EyeIcon className="text-primary size-3.5 shrink-0" aria-hidden />
        ) : null}
        <span>{label}</span>
        {highlightedHint ? (
          <span className="text-primary/80 text-[10px]">{highlightedHint}</span>
        ) : null}
      </p>
      <div
        className={cn(
          "relative aspect-[4/3] overflow-hidden rounded-lg border border-dashed transition-colors",
