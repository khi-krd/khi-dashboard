...
import { CollectionCoverThumb } from "@/components/image-collections/collection-cover-thumb"
import { CollectionListLangChips } from "@/components/image-collections/collection-language-chip"
...
function TableSortLabel({
  ...
  return (
    <span className="inline-flex cursor-pointer items-center gap-1.5">
      {label}
      {sorted === "asc" ? (
        <ChevronUpIcon className="text-muted-foreground size-3.5" aria-hidden />
      ) : sorted === "desc" ? (
...
function CollectionsListPaginationFooter() {
...
export function CollectionsDataGrid({
  rows,
  pagination,
  onPaginationChange,
  sorting,
  ...
  onDeleteOne: (row: CollectionAdminTableRow) => void
}) {
  const columns = useMemo<ColumnDef<CollectionAdminTableRow>[]>(
    () => [
      {
        id: "cover",
        enableSorting: false,
        ...
        header: NS.col.cover,
        cell: ({ row }) => <CollectionCoverThumb collection={row.original} />,
      },
      {
        id: "titleCkb",
      ...
      },
      {
        ...
        header: NS.col.type,
        cell: ({ row }) => (
          <CollectionTypePill collectionType={row.original.collectionType} />
        ),
      },
      {
        id: "topic",
      ...
      {
        id: "items",
        accessorKey: "sortItems",
        sortingFn: "basic",
        size: 80,
        meta: {
          headerTitle: NS.col.items,
          cellClassName: "w-20",
        ...
        header: ({ column }) => (
          <button
            ...
            onClick={() => column.toggleSorting()}
          >
            <TableSortLabel label={NS.col.items} sorted={column.getIsSorted()} />
          </button>
        ),
        cell: ({ row }) => (
          <span className="text-muted-foreground inline-flex items-center gap-1 font-mono text-xs">
            ...
            {row.original.itemCount > 0
              ? formatCkbDigits(row.original.itemCount)
              : NS.dash}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-muted-foreground line-clamp-2 text-xs">
            {row.original.ckbContent?.location?.trim() || NS.dash}
          </span>
        ),
      },
      {
        ...
        size: 128,
        meta: {
          headerTitle: NS.col.collected_by,
          cellClassName: "w-32 hidden md:table-cell",
          skeleton: <Skeleton className="h-3 w-20" />,
        },
        header: NS.col.collected_by,
        cell: ({ row }) => (
          <span className="text-muted-foreground line-clamp-2 text-xs">
            {row.original.ckbContent?.collectedBy?.trim() || NS.dash}
          </span>
        ),
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <CollectionListLangChips languages={row.original.contentLanguages ?? []} />
        ),
      },
      {
        id: "actions",
  ...
  return (
    ...
    >
      <DataGridContainer border={false} className="overflow-hidden rounded-lg border">
        ...
        <CollectionsListPaginationFooter />
      </DataGridContainer>
    </DataGrid>
  )
}
