...
import { useCollectionTopicsQuery } from "@/hooks/useImageCollections"
import type { TopicDto } from "@/types/image-collections"

export function CollectionTopicCombobox({
  topicId,
  newTopic,
  onSelectTopic,
  onClearTopic,
  ...
  return (
    <div className="space-y-2">
        ...
        <PopoverContent className="w-72 p-2" align="start">
            ...
            {topics.map((t) => (
              <li key={t.id}>
                ...
                >
                  ...
                  {t.nameKmr ? (
                    <span className="text-muted-foreground block text-xs">{t.nameKmr}</span>
                  ) : null}
                </button>
              </li>
            ))}
          ...
          {showCreate ? (
            <div className="border-border mt-2 space-y-2 border-t pt-2">
              <Input
                placeholder={NS.lang.ckb}
                value={nameCkb}
                onChange={(e) => setNameCkb(e.target.value)}
              />
              <Input
                placeholder={NS.lang.kmr}
                value={nameKmr}
      ...
      <p className="text-muted-foreground text-xs">{NS.topic.helper}</p>
    </div>
  )
}
