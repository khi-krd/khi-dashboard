    ...
    <div className="flex min-w-0 flex-col gap-0.5 leading-tight">
      ...
      {third ? (
        <span className="text-muted-foreground/70 flex items-center gap-1 text-[10px]">
          ...
          <span className="line-clamp-1">{third}</span>
        </span>
      ) : null}
    </div>
  )
...
function SoundsListPaginationFooter() {
...
export function SoundsDataGrid({
  rows,
  pagination,
  onPaginationChange,
  sorting,
    ...
    () => [
      {
        ...
        meta: {
          ...
          skeleton: <Skeleton className="size-12 shrink-0 rounded-md" />,
        },
        header: NS.col.cover,
        cell: ({ row }) => (
          <SoundCoverThumb
            sound={row.original}
            previewSrc={getFirstPlayableUrl(row.original)}
            previewTitle={row.original.titleCkb}
          />
        ),
      },
      {
      ...
      },
      {
        ...
        header: NS.col.type,
        cell: ({ row }) => (
          <span className="text-sm">{row.original.soundType ?? NS.dash}</span>
        ),
      },
      {
        id: "state",
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          if (isSingleOne) {
            return (
              <span className="text-muted-foreground font-mono text-xs tabular-nums">
                {d != null && d > 0 ? formatDuration(d) : NS.dash}
              </span>
            )
          }
          return (
      ...
      },
      {
        ...
        size: 80,
        meta: {
          headerTitle: NS.col.files,
          cellClassName: "w-20",
          skeleton: <Skeleton className="h-3 w-12" />,
        },
        header: NS.col.files,
        cell: ({ row }) => {
          const count = row.original.fileCount
          const isSingleOne =
            row.original.trackState === "SINGLE" && count === 1
          if (isSingleOne) return null
          return (
            <span className="text-muted-foreground text-xs">
              {count > 0 ? NS.files.count(formatCkbDigits(count)) : NS.dash}
            </span>
          )
        },
      },
