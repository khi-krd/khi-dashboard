...
export function NewsBreadcrumbBar({
  segments,
  className,
}: {
      ...
      {segments.map((seg, i) => {
        ...
        return (
          <span key={`${seg.label}-${i}`} className="flex items-center gap-1.5">
            {i > 0 ? (
              <ChevronRightIcon
                className="text-muted-foreground/50 size-6 shrink-0 rtl:rotate-180"
                aria-hidden
              />
            ) : null}
            {seg.href && !isLast ? (
...
export function dashboardNewsCrumbHref() {
  return "/dashboard"
}

export function newsListCrumbLabel() {
  return NS.page.title
}
